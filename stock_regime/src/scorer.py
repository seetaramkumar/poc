...
============================
Phase 3 change in _build_continuous_scores:
  instability_score now reads volatility_instability_score directly from the
  snapshot (pre-computed composite in indicators.py), instead of re-computing
  it from the individual component fields. This eliminates the double-computation
  and ensures the scorer uses the same calibrated value as signals.py.

VOLATILE regime scoring (score_regimes) is unchanged — it still uses the
boolean signals (volatile_instability, atr_high, high_reversal_freq) produced
by signals.py after the anti-volatile guard has been applied. This means a
strong trend stock whose volatile_instability was suppressed by the guard will
also score 0 for VOLATILE here.
"""
...
from .models import (
    ContinuousScores, DimensionalScores, MarketRegimeInput,
    StockIndicatorSnapshot, StockRegime, StockSignals,
)
...
class StockRegimeScorer:
    ...
    def score_regimes(self, signals: StockSignals) -> dict[StockRegime, float]:
        """
        Boolean dot-product scoring per regime.

        VOLATILE scoring requires volatile_instability (Phase 3 composite signal)
        as the primary component. atr_high alone is insufficient.
        """
        sig = signals
        sc  = self.cfg.scoring
        i   = int
        ...
        return {
            ...
            # VOLATILE requires volatile_instability (composite) as primary.
            # atr_high alone (expanding ATR on a clean trend) does NOT trigger VOLATILE.
    ...
    def score_dimensions(
        ...
        # Volatility: ATR expansion + instability bonus - trend penalty
        # instability_score now comes directly from the pre-computed composite
    ...
    # ── Phase 3: Market-context score adjustment ────────────────────
    ...
    def apply_market_context(
        ...
        if mr == "BEARISH_TREND":
            _adj(StockRegime.TREND_UP,   0.85)
            _adj(StockRegime.MOMENTUM,   0.85)
            _adj(StockRegime.TREND_DOWN, 1.15)
    ...
    def _build_continuous_scores(self, snap: StockIndicatorSnapshot) -> ContinuousScores:
        ...
        # EMA alignment: three sub-components
        ema_align = 0.0
        ...
        ema_dist = (
            _c((snap.ema_distance_pct + 0.20) / 0.40)
            if snap.ema_distance_pct is not None else 0.5
        )

        # ATR expansion: 0.5→0, 1.0→0.5, 2.0→1.0
        atr_score = 0.5
        ...
        # RS trend slope
        rs_trend = (
            _c((snap.rs_trend + 0.005) / 0.010)
            if snap.rs_trend is not None else 0.5
        )

        # ROC: -5%→0, 0%→0.5, +5%→1.0
        roc_score = (
            _c((snap.roc_10 + 5.0) / 10.0)
            if snap.roc_10 is not None else 0.5
        )
        ...
        # Phase 3: instability_score — read directly from pre-computed composite.
        # No re-computation here. This ensures scorer and signals.py use the same value.
        instability_score = (
            float(snap.volatility_instability_score)
            if snap.volatility_instability_score is not None else 0.0
        )

        # Phase 2: ranging score from directional efficiency + BB width + EMA spread
        ranging_components = []
